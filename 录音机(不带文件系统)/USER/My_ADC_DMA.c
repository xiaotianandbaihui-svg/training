...
uint8_t a[2*DATA_BLOCK]={0};
uint8_t a2[2*DATA_BLOCK]={0};
...
void Re_Init()
{
	if(flag==1)
	{
		...
		LCD_ShowNum(0,0,timer*25*DATA_BLOCK/1000000,5,16);
//		LCD_ShowString(3*8,0,8*6,16,16,".");
		LCD_ShowChar(24+24,0,'.',16,0);
		for(int i=0;i<6;i++)
		{
					LCD_ShowNum((4+3+i)*(8),0,((timer*25*DATA_BLOCK)%show(6-i))/show(6-i-1),1,16);
		}

		flag=0;
		if(swap==1)
		{
//			My_Flash_Prog(DATA_BLOCK,&Base,Memory);
			uint8_t* temp1=(uint8_t*)Memory;
				SD_WriteMultiBlocks(temp1,0+(timer-1)*DATA_BLOCK*2,512,DATA_BLOCK*2/512);
		}
		if(swap==0)
		{
			uint8_t* temp2=(uint8_t*)Memory2;
							SD_WriteMultiBlocks(temp2,0+(timer-1)*DATA_BLOCK*2,512,DATA_BLOCK*2/512);
//						My_Flash_Prog(DATA_BLOCK,&Base,Memory2);
		}
		if(timer==0)
		{
			TIM_Cmd(TIM8,DISABLE);
...
void TIM8_Init_For_ADC()
{
	...
	TIM_TimeBaseInitStructure.TIM_Period=25-1;
...
//			DAC_DAM_Init();		
				...
				{
					swap_dac=1;
					SD_ReadMultiBlocks(a2,times*DATA_BLOCK*2,512,DATA_BLOCK*2/512);
				}
				else
				{
					swap_dac=0;
					SD_ReadMultiBlocks(a,times*DATA_BLOCK*2,512,DATA_BLOCK*2/512);
...
//	{
//		is_have++;
//		for(int i=0;i<time_for_record*DATA_BLOCK;i++)
	/*经过测试，最后这两秒的数据总会丢失，但是写入成功，就是无法读取，使用代码直接读取也失败的。其实这就是写入耗费的时间和读取时间不一致产生的结果*/
//		LCD_ShowNum(0,200,*((__IO uint16_t*)Sub_Base+39*DATA_BLOCK),5,16);
//	}
					LCD_ShowNum(0,220,DAC->DHR12R1,5,16);
			LCD_ShowNum(0,240,times*25*DATA_BLOCK/1000000,5,16);
	if(key==1)
	{
		if(state==0)
		{
			if(is_have==1)
			{
					SD_ReadMultiBlocks(a,0,512,DATA_BLOCK*2/512);
